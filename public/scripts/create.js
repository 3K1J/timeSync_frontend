$(document).ready(() => {
  ...
  function createTimesForm() {
    ...
    $('#dateDiv').append($('<input>', {type: 'date', class: 'validate datepicker', placeholder: 'Please choose a Date', class: 'times', id: 'date'}))
    $('#inputForm').append($('<div>', {id: 'startDiv', class: 'input-field col s12 divContainer'}))
    $('#startDiv').append($('<input>', {type: 'text', name: 'validate startTime', placeholder: 'Please choose a Start Time', id: 'start', class: 'times'}))
  ...
  $(document).on('click', '#submitCreateButton', function () {
    if ($('.invChoice').length === 0) {
      $('#submitCreateButton').text('Make sure you add some People')
    } else {
      var eventSend = {}
